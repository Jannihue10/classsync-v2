...
// Gemeinsames Beitreten/Erstellen-Formular für Onboarding (klassenlos) UND AddKlasseModal
// (weitere Klasse hinzufügen). createKlasse/joinByCode fügen die Klasse zu klasseIds hinzu
// und schalten sie aktiv; der klassen-skopierte Baum (inkl. AppShell) remountet danach.
export default function KlasseForm({ uid, onDone }) {
  const { t } = useTheme();
  ...
  async function handleCreate(e) {
    ...
    setBusy(true);
    try {
      await createKlasse(klassenName, uid);
      onDone?.();
    } catch (err) {
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 4, background: t.surface2, borderRadius: radius.md, padding: 4, marginBottom: 20 }}>
        <button type="button" style={tabStyle(tab === "join")} onClick={() => { setTab("join"); setError(""); }}>
          Klasse beitreten
        </button>
        <button type="button" style={tabStyle(tab === "create")} onClick={() => { setTab("create"); setError(""); }}>
          Klasse erstellen
        </button>
      </div>

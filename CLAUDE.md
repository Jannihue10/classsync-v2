...
- Firestore-Struktur: `/klassen/{id}/kurse/{id}/{materialien|hausaufgaben|pruefungen|chat}` + `/klassen/{id}/{sammlungen|ankuendigungen}/{id}`.
- **Ankündigungen** sind der einzige **klassenweite** Kanal (alles andere ist kurs-skopiert). `kursIds` daran ist ein **Zielgruppen-Filter, keine Zugriffsschranke** – gefiltert wird clientseitig in `sichtbarFuer()` (`lib/ankuendigungActions.js`), genau wie Materialien/Chat, die per Rules ebenfalls jedes Klassenmitglied lesen darf. Auto-Löschung nach 90 Tagen macht der Vercel-Cron `api/purge-ankuendigungen.js`.
